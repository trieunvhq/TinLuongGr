    ...
    public partial class frmBaoCao_Nhap_Xuat_ton_kho_DaiLy : Form
    {
        ...
        private DataTable LoadDaTa_TonDauKy(DateTime xxtungay )
        {
            ...
            for (int i = 0; i < dt_XuatTruoc.Rows.Count; i++)
            {
                ...
                if (rows.Length == 0)
                {
                    SoLuong_XuatTruocKy = Convert.ToDouble(dt_NhapTruoc.Rows[i]["SoLuong_XuatTruocKy"].ToString());
                    GiaTri_XuatTruocKy = Convert.ToDouble(dt_NhapTruoc.Rows[i]["GiaTri_XuatTruocKy"].ToString());
