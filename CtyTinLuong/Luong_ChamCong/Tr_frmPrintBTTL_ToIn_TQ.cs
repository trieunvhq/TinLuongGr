...
{
    public partial class Tr_frmPrintBTTL_ToIn_TQ : Form
    {
        private int _thang;
        private int _nam;
        private DataTable _data;

        public Tr_frmPrintBTTL_ToIn_TQ(int thang, int nam, DataTable data)
        {
            _data = data;
            _thang = thang;
            _nam = nam;
        ...
        private void Tr_frmPrintBTTL_ToIn_TQ_Load(object sender, EventArgs e)
        {
            Tr_PrintBTTL_ToIn_TQ xtr111 = new Tr_PrintBTTL_ToIn_TQ(_thang, _nam);
            ...
            for (int i = 0; i < _data.Rows.Count; ++i)
            {
                ...
                _ravi["Cong"] = _data.Rows[i]["HinhThuc"].ToString();
                _ravi["PhuCapBaoHiem"] = _data.Rows[i]["PhuCapBaoHiem"].ToString();
