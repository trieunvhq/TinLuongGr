...
{
    public partial class frmChamCong_TDK_Old : UserControl
    { 
        ...
        public frmChamCong_TDK_Old(int id_bophan, frmQuanLy_Luong_ChamCong frmQLLCC)
        {
            _frmQLLCC = frmQLLCC;
        ...
        private void frmChamCong_TDK_Old_Load(object sender, EventArgs e)
        {
        }
        ...
        {
            int _id_vthh = (int)cbHangHoa.SelectedValue;
            if (_id_vthh == 0)
            {
            }
            else
            { 
                ThemMotCongNhanVaoBang(_id_vthh, cbHangHoa.Text, true);
            }
        } 
        private void ThemMotCongNhanVaoBang(int id_vthh_, string ten_, bool isNew)
        {
            int stt_ = 0;
            //
            ...
            for (int i = 0; i < _data.Rows.Count-1; ++i)
            {
                ...
                {
                    MessageBox.Show("Đã tồn tại tên hàng này trong bảng");
                    return;
                }
            }
