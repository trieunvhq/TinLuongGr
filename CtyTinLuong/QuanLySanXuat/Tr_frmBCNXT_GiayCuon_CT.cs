...
{
    public partial class Tr_frmBCNXT_GiayCuon_CT : Form
    {
        ...
        public Tr_frmBCNXT_GiayCuon_CT(DateTime dateStart, DateTime dateEnd, int Idvthh, string MaHang, string TenHang)
        {
        ...
        private void Tr_frmBCNXT_GiayCuon_CT_Load(object sender, EventArgs e)
        {
            try
            {
                LoadData(false);
        ...
        private ModelNXT_InCat_ChiTiet getNXT(DateTime NgaySX, DataTable dtNhap, DataTable dtXuat)
        {
            ...
            foreach (DataRow item in dtXuat.Rows)
            {
                if (NgaySX == Convert.ToDateTime(item["NgayThang"].ToString()))
                {
                    ...
                    TenHang = item["TenVTHH"].ToString();
                    dienGiai = item["DienGiai"].ToString();
                    Xuat += CheckString.ConvertToDouble_My(item["SoLuong"].ToString());
        ...
        private void gridControl2_DoubleClick(object sender, EventArgs e)
        {
            ...
            //    if(_MaNhanVien != "")
            //    {
            //        Tr_frmQuanLyDML_CongNhat ff = new Tr_frmQuanLyDML_CongNhat(id_congnhan_, "Tr_frmBCNXT_GiayCuon_CT", this);
            //        ff.Show();
            //    }
            //}
            //catch (Exception ee)
